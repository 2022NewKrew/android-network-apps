...
class PostsPagingSourceTest {
    ...
    private val data = Gson().fromJson<List<Post>>(
        jsonPostString,
        object : TypeToken<ArrayList<Post>>() {}.type
    )

    @Test
    fun pagingTest() = runBlocking {
        // Mock 데이터 검증
        assertEquals(1, data[0].id)
        assertEquals("qui est esse", data[1].title)

        // 결과 정의
        `when`(api.getPosts(page = 1, limit = 20)).thenAnswer {
            data.subList(0, 20)
        }
        `when`(api.getPosts(page = 2, limit = 20)).thenAnswer {
            data.subList(20, 40)
        }
        `when`(api.getPosts(page = 3, limit = 20)).thenAnswer {
            data.subList(40, 60)
        }

        val pagingSource = PostPagingSource(api)
