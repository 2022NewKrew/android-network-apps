...
class PhotoRemotePagingSourceTest {
    @Mock
    private lateinit var api: PhotoApi

    @Test
    fun 페이징_로드_테스트() = runBlocking {
        val datas = Gson().fromJson<List<Photo>>(
            jsonArrayString,
            object : TypeToken<ArrayList<Photo>>() {}.type
        )

        // Mock 데이터 검증
        assertEquals(1, datas[0].id)
        assertEquals("reprehenderit est deserunt velit ipsam", datas[1].title)

        // 결과 정의
        `when`(api.getPhotos(page = 1)).thenAnswer {
            datas.subList(0, 10)
        }
        `when`(api.getPhotos(page = 2)).thenAnswer {
            datas.subList(11, 20)
        }
        `when`(api.getPhotos(page = 3)).thenAnswer {
            datas.subList(21, 30)
        }

        val pagingSource = PhotoRemotePagingSource(api)

        // 1페이지
        assertEquals(
            PagingSource.LoadResult.Page(
                data = datas.subList(0, 10),
                prevKey = null,
                nextKey = 2
            ),
            pagingSource.load(
